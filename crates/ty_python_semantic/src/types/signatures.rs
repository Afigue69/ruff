...
impl<'db> Signature<'db> {
    ...
    pub(crate) fn bind_self_to(
        ...
        let receiver_annotation_accepts_self = |expected_self_ty| {
        ...
        if inferable_typevars == InferableTypeVars::None || !expected_self_ty.has_typevar(db) {
            if receiver_annotation_accepts_self(expected_self_ty) {
            ...
            let bind_if_valid = |specialization| {
                ...
                receiver_annotation_accepts_self(specialized_expected_self_ty).then(|| {
    ...
    ///
    /// This is the lightweight receiver-applicability check used by call binding before the
    /// receiver is actually baked into the candidate overload. If a signature has no positional
    /// first parameter, we conservatively keep it.
